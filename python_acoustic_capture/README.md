...
本包为 **0.2.12 RIR 重构版**，基于 `codex/rir-asio-audit-20260905` 分支提交
`a29a30be2ad83388619267643711d0c7be8a44c0`。启动、配置隔离、音频帧完整性、
延迟计算、裁剪边界和 RIR 保存已作修复；详见 [重构说明](REFACTOR_NOTES.md)。
这是 Python 源码包，运行环境需要安装依赖；原分支的旧 portable 程序不属于本次构建产物。
...
采集在后台线程执行；运行日志默认收起，需要排错时再勾选“显示运行日志”。
...
- `音频 / 语音采集`：把普通播录和语音增强合并。默认“标准监督：目标 + 干扰 + MIXED”在一次连续
  播录中采 `target_only + interferer_only + mixture`；纯干扰可带回服务器做离线混合。默认下拉框
  只列标准监督、只采干净目标、只采干扰和仅录制四项。含底噪、普通单文件同步播录、仅播放及
  自定义组合仍保留在“更多设置”中。界面只显示当前方案实际需要的文件、声源和通道。

顶部工具条只保留“打开测试清单”和“检查声卡”；打开/保存配置、设备详情、配置检查和标签导入放在
菜单中。结果文件选择、试听选项和绘图工具栏仅在完成采集或打开历史结果后显示，采集准备阶段只保留
播录状态卡、进度和当前任务示意。
...
配对数量严格等于“随机测量次数”，不再等于文件夹大小，也不提供笛卡尔积选项。大素材库会在后台扫描，
开始采集时只读取、重采样并计算当前一对文件的 SHA256，不会先把整个目录的音频读进内存。例如从
2000 条 target 和 2000 条 interferer 中测量 100 次，只形成 100 组任务，不会展开成 4,000,000 组。

正式开始前会逐个读取音频文件头，损坏或空音频会在扬声器发声之前报出。每完成一组配对，
程序都会立即落盘标签和断点；界面在开始前显示预计最短总时长，采集中显示完成数和预计剩余时间。
可以随时点击停止。停止、驱动报错或断电后，用同一实验名重新开始，GUI 会恢复原随机种子、原测量次数，
并询问是否从第一条未完成任务续采，不会重新配对。预检还会按片段长度、通道数和播放参考设置估算
整批磁盘占用，空间不足时直接阻止开始。
...
每次单文件 repetition 开始时单独录一次；文件夹模式默认只录一次，供本实验的全部样本引用。
...
repeats:
  ...
  reconstruction_policy: diagnostic
  minimum_reconstruction_correlation: 0.90
  maximum_reconstruction_nmse_db: -10.0
```

`diagnostic` 允许有意截取早期 RIR，重构指标仅供诊断，训练推荐范围明确限定为录音和重复性。
若需要 RIR 解释完整录音响应，设 `reconstruction_policy: full_response`；每个 take 及最终平均
结果都必须满足所配相关性和绝对 NMSE 条件。阈值可按实验要求修改，不做隐式幅度归一化。
仅有两次且彼此不一致时，两次均保留原始数据并退出接受集合，需要补采，不再强行判为一致。

延迟频带、最大时差和一致性容差会实际参与 GCC-PHAT 与低频群延迟计算；没有足够能量、
搜索范围不足或相位分支不明确时会标记无效。ESS 分段 ppm 只表示声学时序趋势，不能单独确认
声卡时钟漂移。裁剪窗口必须覆盖各通道直达声与请求的尾长，否则该 take 会被拒绝并保留诊断文件。
音频流只接受 `float32`；RIR、逆滤波器与处理后响应固定保存为 FLOAT，原始录音仍遵循
`storage.wav_subtype`，避免系统增益大于 1 时被 PCM 削波。

默认严格录制 5 次，不使用收敛提前停止。每次原始录音、完整反卷积 IR、对齐裁剪 IR、ESS 回卷重构信号
和指标都会保存。程序先完成每次录音的丢帧、非有限值、静音通道、削波、扫频信噪比和重复通道检查，
全部次数结束后再从基础质检通过的结果中选择早期 RIR 中位代表（medoid）作为公共基准。每个 take 的
所有麦克风只做同一个时间平移；与重复测量共识不一致的 take 不进入均值，但原始文件不会删除。
公共到达峰跨次漂移只会给出声卡时钟警告，不会被误当作麦克风间的物理时差或单独作为拒绝条件。
...
- `metrics/take_NNN.json`：削波、直达声、公共漂移、早期 RIR 共识相关性、GCC-PHAT、低频群延迟和后端状态；
- `metrics/summary.json`：接受/拒绝的 take 编号、选择方法，以及每个原始 take 的离线重选路径。

去卷积使用频率相关 Kirkeby 正则化逆滤波。生成的 ESS 和反卷积结果已与本机 MATLAB R2024b
`sweeptone` / `impzest` 做数值对照：同一组输入的 ESS 最大绝对误差约 `1.49e-8`，RIR 最大绝对误差约
`2.46e-8`。正则化会有意牺牲少量绝对幅值，以降低扫频带外噪声被反向放大的风险，因此不能把它换成
看似幅值更接近 1、但噪声增益很大的普通逆滤波。第 1 个麦克风的首次显著到达提供公共裁剪基准；
所有麦克风只做同一个时间平移，因此真实的麦克风间到达时差（TDOA）会保留。每次还会保存未裁剪的
`take_NNN_full_ir.wav`，并分别报告局部峰值偏移、GCC-PHAT 延迟和低频群延迟。

如果录制和播放来自两个不同的非 ASIO 端点，即使扬声器是有线连接、两个端点也选择了同一个 MME 或
WASAPI 协议，也不代表它们共用硬件时钟。此时绝对峰值包含驱动、转换器和声学传播的总延迟，长扫频还
可能因采样时钟偏差而失真。正式 RIR 测量应优先把麦克风输入和扬声器输出都接到同一块 RME，并使用同一
ASIO 双工设备；如果要测绝对传播时间，还必须另加有线回环或参考通道。

Windows ASIO 采用单线程所有权：创建流、启动、停止和关闭都由同一个音频工作线程完成，GUI
只设置停止标志，不会从 Tk 线程或额外的“abort 线程”直接调用 ASIO。正式采集开始前，右侧试听流会
先关闭，采集中试听和再次查询声卡也会禁用，以免单客户端 ASIO 驱动被第二条流抢占。关闭程序时若仍在
录制，界面会先等待声卡安全关闭。这里的“工作线程”仍属于同一个 GUI 进程，并不是后台播放进程。
...
实验室使用 ASIO 时优先解压文件夹版并运行其中的 `AcousticCapture.exe`：它只有一个应用进程，
不会经过单文件版的临时解包/引导父进程，驱动 DLL 和日志位置也更容易核查。单 EXE 版仍可用于普通
Windows 声卡试采；两种版本内部的实际音频流都遵守同一线程所有权。
