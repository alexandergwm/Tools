...
- `音频 / 语音采集`：把普通播录和语音增强合并。默认“标准监督：目标 + 干扰 + MIXED”在一次连续
  ...
  文件、声源和通道。
- `简单录制`：只显示录制声卡、输入通道和保存目录。点击“开始简单录制”后必须输入本次录音名称，
  程序会在保存目录中生成 `日期时间_录音名称.wav`；录到需要的长度后手动停止。

简单录制不要求播放设备、时长、ESS 参数、语音素材或标签。录音采用流式写盘，点击
`停止录制 / 测试` 后右侧会自动显示刚录好的多通道波形；录制时长不受内存容量限制，但应保证
保存磁盘有足够空间。
...
文件夹批量模式只需要填写 `measurement_count`（GUI 中为“随机测量次数”）；它就是本次实验的任务总数，
不会再乘一个隐藏的重复次数。旧的 `repetitions` 只为单文件兼容流程保留。各项之间物理位置和输入增益
必须保持不变。
...
在“音频 / 语音采集”中选择语音采集方案，把“素材选择方式”设为“文件夹批量”，分别选择目标语音目录
和干扰声音目录，然后只填写“随机测量次数”。每次开始新的命名实验，GUI 都会自动生成新的随机种子，
clean 和 interferer 分别洗牌后按序配对；因此每次实验的素材和组合都会变化。相同素材与相同种子仍可
完全复现，实际种子会写入 checkpoint、labels 和配对表。素材不足时使用新的 shuffle bag 轮次复用。
